...
from scripts.slice2seg import prepare_for_first_stage, dice_score, iou_score,precision_score,recall_score
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.noise_lstm = NoiseLSTM(**noise_lstm_config)

        # 添加动态噪声参数
        self.dynamic_noise = True  # 是否启用动态噪声
        self.noise_scale = 1.0  # 噪声缩放因子
        self.min_noise_std = 0.1  # 最小噪声标准差
        self.max_noise_std = 2.0  # 最大噪声标准差
    
    def encode_with_vae(self, x):
        """使用VAE编码器提取均值和方差"""
        if self.vae is None:
            return None, None
        with torch.no_grad():
            ...
            if isinstance(posterior, DiagonalGaussianDistribution):
                mu, logvar = posterior.mu, posterior.logvar
                std = torch.exp(0.5 * logvar)
            else:
                mu, std = posterior.mean, posterior.stddev
        return mu, std
    
    def generate_dynamic_noise(self, x_start, t=None, mu=None, std=None, noise_id=0):
        """基于VAE特征的多尺度动态噪声生成
        Args:
            x_start: 原始输入图像 [B,C,H,W]
            t: 时间步 (可选)
            mu: 预先计算的均值 (可选)
            std: 预先计算的标准差 (可选)
            noise_id: 噪声样本ID (用于多噪声生成)
        Returns:
            动态调整的噪声张量
        """
        # 基础高斯噪声
        base_noise = torch.randn_like(x_start)
        
        # 如果禁用动态噪声或没有VAE，返回标准噪声
        if not self.dynamic_noise or self.vae is None:
            return base_noise
        
        # 如果没有提供预计算的mu/std，通过VAE编码器获取
        if mu is None or std is None:
            mu, std = self.encode_with_vae(x_start)
            if mu is None or std is None:
                return base_noise
        
        # 多噪声样本调制因子 (基于noise_id产生差异)
        modulation = 0.8 + 0.4 * (noise_id / max(1, self.num_noises-1))  # 在0.8-1.2之间线性变化
        
        # 动态噪声生成 (核心公式)
        dynamic_noise = mu + (self.noise_scale * modulation) * std * base_noise
        
        # 时间步相关的噪声混合 (DDIM/DDPM兼容)
        if t is not None:
            alpha_t = extract_into_tensor(self.alphas_cumprod, t, x_start.shape)
            # 保留原始图像特征的比例随时间减少
            dynamic_noise = alpha_t.sqrt() * dynamic_noise + (1 - alpha_t).sqrt() * base_noise
        
        # 可选：添加局部结构感知的噪声调整
        if self.local_adaptation:
            # 使用3x3卷积提取局部结构
            local_std = F.avg_pool2d(std.pow(2), kernel_size=3, padding=1).sqrt()
            dynamic_noise *= (local_std / (std + 1e-6)).clamp(0.5, 2.0)
    ...
    def p_losses(self, x_start, t, noise=None):
        # 使用VAE编码器提取图像特征（只计算一次）
        with torch.no_grad():
            if hasattr(self, 'vae') and self.vae is not None:
                posterior = self.vae.encode(x_start)
                mu = posterior.mean if hasattr(posterior, 'mean') else posterior.mu
                std = posterior.stddev if hasattr(posterior, 'stddev') else torch.exp(0.5 * posterior.logvar)
            else:
                mu, std = None, None
        
        # 生成多个动态噪声样本
        num_noises = 10  # 5个噪声样本
        total_diffusion_loss = 0
        consistency_pairs = []  # 用于存储一致性损失的预测对
        
        for i in range(num_noises):
            # 为每个样本生成独特的动态噪声
            noise = self.generate_dynamic_noise(x_start, t, mu, std, noise_id=i)
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
            model_out = self.model(x_noisy, t)
            
            # 计算基础损失
            target = noise if self.parameterization == "eps" else x_start
            diffusion_loss = self.get_loss(model_out, target, mean=False).mean([1, 2, 3])
            total_diffusion_loss += diffusion_loss
            
            # 收集奇数索引的预测用于一致性计算
            if i % 2 == 1 and self.consistency_weight > 0:
                consistency_pairs.append(model_out.detach())
        
        # 平均扩散损失
        avg_diffusion_loss = total_diffusion_loss / num_noises
        
        # KL散度损失（基于VAE）
        kl_loss = 0.0
        if hasattr(self, 'vae') and self.vae is not None and hasattr(posterior, 'kl'):
            kl_loss = posterior.kl().mean([1, 2, 3])
        
        # 多噪声一致性损失（比较相邻噪声级别的预测）
        consistency_loss = 0.0
        if len(consistency_pairs) > 1 and self.consistency_weight > 0:
            for i in range(len(consistency_pairs)-1):
                consistency_loss += F.mse_loss(consistency_pairs[i], consistency_pairs[i+1])
            consistency_loss /= (len(consistency_pairs)-1)
        
        # 总损失计算
        total_loss = (
            avg_diffusion_loss.mean() * self.l_simple_weight +
            kl_loss.mean() * self.kl_weight +
        ...
        loss_dict = {
            f'{log_prefix}/loss_simple': avg_diffusion_loss.mean(),
            f'{log_prefix}/loss_kl': kl_loss.mean() if hasattr(self, 'vae') else 0.0,
            f'{log_prefix}/loss_consistency': consistency_loss,
            f'{log_prefix}/loss_total': total_loss,
        }
        
        # 添加VLB损失
        if self.original_elbo_weight > 0:
            loss_vlb = (self.lvlb_weights[t] * avg_diffusion_loss).mean()
            total_loss += self.original_elbo_weight * loss_vlb
    ...
    def configure_optimizers(self):
        ...
        params = list(self.model.parameters())+ list(self.generate_dynamic_noise.parameters())
        if self.learn_logvar:
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # ✅ ADD THIS LINE: initialize unet_sd_keys
        self.unet_sd_keys = set(self.model.diffusion_model.state_dict().keys())
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
            ...
            mask=None, x0=None, shape=None, num_noises=10, **kwargs):
        if shape is None:
        ...
        # 使用多尺度噪声生成模块生成噪声
        noise_input = torch.randn(batch_size, 10, self.noise_lstm.input_dim, device=self.device)
        noise = self.generate_dynamic_noise(noise_input)  # 假设序列长度为 5
        ...
        for i in range(num_noises):
            # 为每个样本生成独特的动态噪声
            if hasattr(self, 'generate_dynamic_noise'):
                # 使用动态噪声生成
                base_noise = torch.randn(shape, device=self.device)
                dynamic_noise = self.generate_dynamic_noise(
                    x_start=torch.randn(shape, device=self.device) if x0 is None else x0,
                    noise=base_noise,
                    noise_id=i
                )
                x_T = dynamic_noise
            ...
            sample, intermediates = self.p_sample_loop(
                cond=cond,
                shape=shape,
                return_intermediates=return_intermediates,
                x_T=x_T,
        ...
        # 在聚合部分替换为：基于边界清晰
        samples_stack = torch.stack(samples_list)  # [num_noises, B, C, H, W]

        # 计算边界清晰度权重
        boundary_weights = []
        for sample in samples_stack:
            # 使用梯度计算边界清晰度
            gradient_x = torch.abs(sample[:, :, :, 1:] - sample[:, :, :, :-1])
            gradient_y = torch.abs(sample[:, :, 1:, :] - sample[:, :, :-1, :])
            boundary_strength = (gradient_x.mean(dim=(1,2,3)) + gradient_y.mean(dim=(1,2,3))) / 2
            
            # 边界越清晰，权重越高（适用于医学图像）
            boundary_weights.append(boundary_strength)

        boundary_weights = torch.stack(boundary_weights)  # [num_noises, B]
        weights = torch.softmax(boundary_weights, dim=0)

        boundary_aware_pred = (samples_stack * weights.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)).sum(dim=0)
        if return_intermediates:
            # 返回平均预测和所有中间结果
            return boundary_aware_pred, all_intermediates
        
        return boundary_aware_pred
    ...
    def log_dice(self, data=None, save_dir=None, ddim_steps=50):
            ...
            def get_dice_loop(data, sampler, use_direct=False, noise=None, save_dir=None, ddim_steps=50):
                ...
                for prompts in pbar:
                    ...
                    for idx in range(1, self.num_classes):
                        ...
                        metrics_list[2].append(precision_score(prediction == idx, label == idx))
                        metrics_list[3].append(recall_score(prediction == idx, label == idx))
                    dice_list += np.array(metrics_list[0])
                    iou_list += np.array(metrics_list[1])
                    precision_list += np.array(metrics_list[2])
                    recall_list += np.array(metrics_list[3])
                pbar.close()
                ...
                precision_list = precision_list / len(data)
                for idx in range(1, self.num_classes):
                    print(f"\033[31m[Mean  precision][cls {idx}]: {iou_list[idx-1]}\033[0m")

                recall_list = recall_list / len(data)
                for idx in range(1, self.num_classes):
                    print(f"\033[31m[Mean  recall][cls {idx}]: {iou_list[idx-1]}\033[0m")


                return dice_list, iou_list, precision_list, recall_list, seg_label_pair
            ...
            with torch.no_grad():
                with precision_scope("cuda"):
                    with self.ema_scope(f"EMA Seg Validation ({used_sampler})"):
                        ema_dice_list, ema_iou_list, ema_prec_list, ema_rec_list, seg_label_pair = get_dice_loop(data, sampler,
                                                           use_direct=True if used_sampler == "direct" else False,
                                                           save_dir=save_dir, ddim_steps=ddim_steps)
            return ema_dice_list, ema_iou_list, ema_prec_list, ema_rec_list,seg_label_pair

        # try dice methods
        ema_dice, ema_iou, ema_prec, ema_rec, seg_label_pair = get_dice(data, used_sampler="direct", save_dir=save_dir)
        multi_dice, multi_iou = np.array(ema_dice), np.array(ema_iou)
        multi_prec, multi_rec = np.array(ema_prec), np.array(ema_rec)
        ...
        metrics_dict.update({"val_avg_precision/direct_ema": np.mean(multi_prec)})
        metrics_dict.update({"val_avg_recall/direct_ema": np.mean(multi_rec)})
        ...
        for cls in range(1, self.num_classes):
            metrics_dict.update({f"val_avg_precision/direct_ema_{cls}": multi_iou[cls-1]})
        for cls in range(1, self.num_classes):
            metrics_dict.update({f"val_avg_recall/direct_ema_{cls}": multi_iou[cls-1]})
        ...
        metrics_dict.update({"val_avg_precision": list(multi_prec)})
        metrics_dict.update({"val_avg_recall": list(multi_rec)})

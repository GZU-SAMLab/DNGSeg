...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        cross_attn = CrossAttention(embed_dim=self.channels,num_heads=8).to(self.device)
        ...
        for i in range(num_noises):
            ...
            # 将生成的样本作为k和v，原始图像信息作为q
            q = x0.reshape(batch_size, self.channels, -1).permute(2, 0, 1)  # (seq_len, batch_size, embed_dim)
            k = model_out.reshape(batch_size, self.channels, -1).permute(2, 0, 1)  # (seq_len, batch_size, embed_dim)
            v = k  # 使用k作为v

            # 通过交叉注意力机制生成新的样本
            attn_output = cross_attn(q, k, v)
            attn_output = attn_output.permute(1, 2, 0).reshape(batch_size, self.channels, self.image_size, self.image_size)
            # 添加残差连接
            attn_output = attn_output + model_out  # 将交叉注意力模块的输出与原始样本相加
            
            # 计算基础损失
            target = noise if self.parameterization == "eps" else x_start
            diffusion_loss = self.get_loss(attn_output, target, mean=False).mean([1, 2, 3])
...
class LatentDiffusion(DDPM):
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        for noise in noises:
            sample, intermediates = self.p_sample_loop(cond,
                                                    shape,
        ...
        # 融合多个样本
        mean_pred = torch.stack(samples_list).mean(dim=0)
        # # 计算各预测与均值的距离
        # distances = torch.norm(samples_list - mean_pred, p=2, dim=(2,3,4))  # [num_noises, B]
        # # 选择最接近均值的预测
        # selected_indices = distances.argmin(dim=0)  # [B]
        # final_pred = torch.stack([preds_stack[i, b] for b, i in enumerate(selected_indices)])
        
        if return_intermediates:
            return mean_pred, intermediates
        return mean_pred

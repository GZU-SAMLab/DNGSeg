...
from scripts.slice2seg import prepare_for_first_stage, dice_score, iou_score, precision_score, recall_score, hd95_score
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if return_original_cond:
            out.append(xc)
        # 打印输入张量的形状
        # print(f"Input shape in get_input: {x.shape}")
        # print(f"Conditioning input shape in get_input: {c.shape}")
    ...
    def log_dice(self, data=None, save_dir=None, ddim_steps=50):
            ...
            def get_dice_loop(data, sampler, use_direct=False, noise=None, save_dir=None, ddim_steps=50):
                ...
                for prompts in pbar:
                    ...
                    for idx in range(1, self.num_classes):
                        ...
                        # Safe HD95
                        # 确保下面这行HD95计算代码存在且没有被注释掉：
                        try:
                            hd_val = hd95_score(pred_mask.astype(np.float32), gt_mask.astype(np.float32), spacing=(1.0, 1.0))
                            
                        except Exception as e:
                            
                            hd_val = np.sqrt(pred_mask.shape[0]**2 + pred_mask.shape[1]**2)
                        
                        metrics_list[4].append(hd_val)  # 确保这行存在！
                        
                        
                    # metrics_list[4].append(hd95_score(prediction == idx, label == idx))
                    ...
                    hd95_list += np.array(metrics_list[4])
                pbar.close()
                ...
                for idx in range(1, self.num_classes):
                    print(f"\033[31m[Mean  precision][cls {idx}]: {precision_list[idx-1]}\033[0m")

                recall_list = recall_list / len(data)
                for idx in range(1, self.num_classes):
                    print(f"\033[31m[Mean  recall][cls {idx}]: {recall_list[idx-1]}\033[0m")

                hd95_list = hd95_list / len(data)
                for idx in range(1, self.num_classes):
                    print(f"\033[31m[hd95][cls {idx}]: {hd95_list[idx-1]}\033[0m")

                return dice_list, iou_list, precision_list, recall_list,hd95_list ,seg_label_pair
            ...
            with torch.no_grad():
                with precision_scope("cuda"):
                    with self.ema_scope(f"EMA Seg Validation ({used_sampler})"):
                        ema_dice_list, ema_iou_list, ema_prec_list, ema_rec_list,ema_hd_list,seg_label_pair = get_dice_loop(data, sampler,
                                                           use_direct=True if used_sampler == "direct" else False,
                                                           save_dir=save_dir, ddim_steps=ddim_steps)
            return ema_dice_list, ema_iou_list, ema_prec_list, ema_rec_list, ema_hd_list, seg_label_pair


        # try dice methods
        ema_dice, ema_iou, ema_prec, ema_rec,ema_hd, seg_label_pair = get_dice(data, used_sampler="direct", save_dir=save_dir)
        multi_dice, multi_iou = np.array(ema_dice), np.array(ema_iou)
        multi_prec, multi_rec = np.array(ema_prec), np.array(ema_rec)
        multi_hd = np.array(ema_hd)
        ...
        metrics_dict.update({"val_avg_hd95/direct_ema": np.mean(multi_hd)})
        ...
        for cls in range(1, self.num_classes):
            metrics_dict.update({f"val_avg_hd95/direct_ema_{cls}": multi_hd[cls-1]})
        ...
        metrics_dict.update({"val_avg_hd95": list(multi_hd)})

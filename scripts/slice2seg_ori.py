...
def main():
    ...
    if opt.dataset == "synapse-b":
        ...
        run = "2025-07-12T18-13-04_experiment_cvc_vae2_10m_47(best)" 
        ...
        run = "2025-11-08T18-03-10_experiment_cvc_vae2_bianjie_l20" 
        print("Evaluate on cvc dataset in binary segmentation manner.")
        opt.config = glob.glob(os.path.join("logs", run, "configs", "*-project.yaml"))[0]
        opt.ckpt = f"logs/{run}/checkpoints/epoch=991-step=121999.ckpt"
        opt.outdir = "outputs/slice2seg-samples-cvc-vae"
        dataset = CVCAll()
    elif opt.dataset == "kseg":
        run = "2025-11-09T14-41-31_experiment_kseg_vae_biajie_l20" 
        print("Evaluate on kseg dataset in binary segmentation manner.")
        opt.config = glob.glob(os.path.join("logs", run, "configs", "*-project.yaml"))[0]
        opt.ckpt = f"logs/{run}/checkpoints/epoch=619-step=123999.ckpt"

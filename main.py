import time
t0 = time.time()
import argparse, os, sys, datetime, glob, importlib, csv, shutil
import numpy as np
import torch
import torchvision
# import faulthandler
...
class MiddleCheckpointCallback(Callback):
    def __init__(self, candidate_dir, selected_dir, monitor, mode, candidate_top_k=50, keep_ranks=None):
        super().__init__()
        ...
        self.keep_ranks = keep_ranks or [25, 40, 50]
    ...
    def on_fit_end(self, trainer, pl_module):
        ...
        for callback in trainer.callbacks:
            if isinstance(callback, ModelCheckpoint) and getattr(callback, "dirpath", None) == self.candidate_dir:
                candidate_callback = callback
                break

        if candidate_callback is None:
            print("MiddleCheckpointCallback: candidate checkpoint callback not found.")
            return

        best_k_models = getattr(candidate_callback, "best_k_models", None)
        if not best_k_models:
            print("MiddleCheckpointCallback: no candidate checkpoints available.")
            return

        ranked_items = list(best_k_models.items())
        reverse = self.mode == "max"
        ranked_items.sort(key=lambda item: float(item[1]), reverse=reverse)

        valid_ranks = []
        for rank in self.keep_ranks:
            if rank < 1:
                continue
        ...
        if not valid_ranks:
            print("MiddleCheckpointCallback: no valid middle ranks to keep.")
            return
        ...
        for filename in os.listdir(self.selected_dir):
            if filename.endswith(".ckpt"):
                os.remove(os.path.join(self.selected_dir, filename))

        selected_source_paths = set()
        for rank in valid_ranks:
            src_path, score = ranked_items[rank - 1]
            selected_source_paths.add(src_path)
            if rank == 50:
                dst_name = "rank50_worst_in_top50.ckpt"
            else:
                dst_name = f"rank{rank}.ckpt"
            dst_path = os.path.join(self.selected_dir, dst_name)
            shutil.copy2(src_path, dst_path)

        for src_path, _ in ranked_items:
            if src_path not in selected_source_paths and os.path.exists(src_path):
                os.remove(src_path)

        print(f"MiddleCheckpointCallback: saved ranks {valid_ranks} to {self.selected_dir} and removed other candidate checkpoints")
...
if __name__ == "__main__":
    ...
    try:
        ...
        middle_checkpoint_cfg = None
        if hasattr(model, "monitor"):
            middle_checkpoint_cfg = OmegaConf.create({
                "candidate_top_k": 50,
                "keep_ranks": [25, 40, 50]
            })
            if "callbacks" in lightning_config and "middle_checkpoint" in lightning_config.callbacks:
                middle_checkpoint_cfg = OmegaConf.merge(middle_checkpoint_cfg, lightning_config.callbacks.middle_checkpoint)
        ...
        if middle_checkpoint_cfg is not None:
            metric_tag = model.monitor.replace("/", "_")
            middle_candidate_dir = os.path.join(ckptdir, "mid_candidates")
            middle_selected_dir = os.path.join(ckptdir, "mid_selected")
            middle_candidate_ckpt_cfg = {
                "target": "pytorch_lightning.callbacks.ModelCheckpoint",
                "params": {
                    "dirpath": middle_candidate_dir,
                    "filename": f"midcand-{{epoch:06}}-{{step:09}}-{metric_tag}={{{model.monitor}:.6f}}",
                    "monitor": model.monitor,
                    "save_top_k": middle_checkpoint_cfg.candidate_top_k,
                    "mode": middle_checkpoint_mode,
                    "save_last": False,
                    "verbose": True,
                }
            }
            if model.monitor == "val_avg_dice":
                middle_candidate_ckpt_cfg["params"]["every_n_train_steps"] = lightning_config.callbacks.image_logger.params.log_dice_frequency
            default_callbacks_cfg.update({
                'middle_candidate_checkpoint': middle_candidate_ckpt_cfg,
                'middle_checkpoint_selector': {
                    "target": "main.MiddleCheckpointCallback",
                    "params": {
                        "candidate_dir": middle_candidate_dir,
                        "selected_dir": middle_selected_dir,
                        "monitor": model.monitor,
                        "mode": middle_checkpoint_mode,
                        "candidate_top_k": middle_checkpoint_cfg.candidate_top_k,
                        "keep_ranks": list(middle_checkpoint_cfg.keep_ranks),
                    }
                }
            })
